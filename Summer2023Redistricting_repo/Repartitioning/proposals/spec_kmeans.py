...
def repartition(partition: Partition, G: nx.Graph, k: int, nodelist: list[Any]) -> Partition:
    '''
        Returns a new partition by embedding the vertices into a higher-dimensional space and partitioning them using
...
def Embed_Vertices(G: nx.Graph, k: int, nodelist: list, nodeindices: dict) -> Callable[[Any], np.ndarray]:
    '''
        Embeds the vertices of `G` into a `k`-dimensional space using the first `k` eigenvalues of `G`'s normalized
        laplacian.
    '''
    # (|V|, |V|) matrices
...
def Radial_Decomposition(F: Callable, k: int, nodelist: list) -> list[set]:
    ...
    # maps (k, *) centers array to an (n, *) array, s.t. each row is repeated n/k times consecutively
    ...
    # assign vertices to parts s.t. distance is minimized and there are an equal number of vertices in each part
